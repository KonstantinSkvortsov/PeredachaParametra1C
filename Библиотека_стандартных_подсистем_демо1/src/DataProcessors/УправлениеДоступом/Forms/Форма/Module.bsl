...
#Если ВебКлиент Или МобильныйКлиент Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Инструмент ""Управление доступом"" не доступен в веб-клиенте, необходимо использовать тонкий клиент.'"));
		Отказ = Истина;
		Возврат;
#КонецЕсли
...
Процедура СписокОткрытие(Элемент, СтандартнаяОбработка)
	...
	ОбщегоНазначенияКлиент.ОткрытьНавигационнуюСсылку("e1cib/list/" + Список);
...
Процедура ПолеВыбораПапкиОткрытие(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ОбщегоНазначенияКлиент.ОткрытьПроводник(ЭтотОбъект[Элемент.Имя]);
КонецПроцедуры
...
Процедура СравнитьВоВнешнейПрограмме(Команда)
	...
	ОбщегоНазначенияКлиент.ЗапуститьПрограмму(СтрокаЗапуска);
...
Процедура СоздатьНовуюИБ(ИмяПодключения, ИмяФайлаШаблона = "")
	...
	Если Отказ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru = 'Строка соединения ИБ ""%1"" указана неверно:'"), ИмяПодключения) + Символы.ПС
